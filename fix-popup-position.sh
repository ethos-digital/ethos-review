...
# Fix positionnement du popup de commentaire

echo "📝 Correction du positionnement des commentaires..."
...
export default function ReviewPage() {
  ...
  // Calcule la position du popup (gauche ou droite selon la position du clic)
  function getPopupPosition(x: number) {
    if (x > 60) {
      // À droite de l'image -> popup à gauche du pin
      return { right: '100%', left: 'auto', marginRight: '10px' }
    }
    // À gauche/centre de l'image -> popup à droite du pin
                  ...
                  <div ref={imageRef} className="relative cursor-crosshair" onClick={handleImageClick}>
                    ...
                    {newComment && (
                      ...
                      >
                        ...
                        <div 
                          className="absolute top-5 bg-zinc-900 rounded-xl shadow-2xl p-4 w-72 border border-zinc-700"
                          style={getPopupPosition(newComment.x)}
                        >
                          {editingName ? (
                            <input 
                              type="text" 
                            ...
                            {editingCommentId === comment.id ? (
                              <div className="mt-2">
                                <textarea
                                  ...
                                  className="w-full bg-zinc-900 border border-zinc-600 rounded-lg p-2 text-sm resize-none focus:outline-none focus:border-violet-500"
                                  rows={2}
                                  autoFocus
                                />
                              ...
                              <>
                                ...
                                <div className="flex items-center gap-3 mt-2">
                                  <button onClick={() => toggleResolved(comment.id)} className="text-xs text-violet-400 hover:text-violet-300">
                                    {comment.is_resolved ? 'Réouvrir' : 'Résolu'}
                                  </button>
                                  <button onClick={() => startEditingComment(comment)} className="text-xs text-zinc-500 hover:text-zinc-300">
                                    Modifier
                                  </button>
                                  <button onClick={() => deleteComment(comment.id)} className="text-xs text-zinc-500 hover:text-red-400">
                                    Supprimer
                                  </button>
                                </div>
                              </>
...
echo ""
echo "Le popup de commentaire se positionne maintenant :"
echo "  - À droite du pin si on clique à gauche/centre (< 60%)"
echo "  - À gauche du pin si on clique à droite (> 60%)"
